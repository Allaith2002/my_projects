    ...
    } else //elseif($_SERVER['REQUEST_METHOD'] == 'POST')
    {
        ...
        if ($isViewValid) {
            ...
            if (isset($data['Houdbaarheidsdatum']) && $this->validateDate($data['Houdbaarheidsdatum'])) {
                ...
                $currentDate = new DateTime($data['Houdbaarheidsdatum']);

                // Set the maximum number of days to extend the houdbaarheidsdatum
                $maxVerlenging = 7;

                // Add the maximum number of days to the current date
            ...
            if ($this->LeverancierModel->UpdateProductdatum($data)) {
                $this->infoMessage = FormatTextHelper::GetInfoMessage("De houdbaarsdatum is gewijzigd", EnumTypeMessage::Success);
